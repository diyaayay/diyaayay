var tl = gsap.timeline()

tl.from("nav h1, nav h4, nav button", {
    y: -40,
    opacity: 0,
    delay: 0.5, 
    duration: 0.5,
    stagger: 0.15
})

tl.from("nav h1 i", {
    rotate: -200,
    duration: 0.54,
    ease: "power1.inOut"
}, "-=0.5")


tl.from(".center-part-1 h1", {
    x: -300,
    opacity: 0,
    duration: 0.5
}, "-=0.3")


tl.from(".center-part-1 p", {
    x: -100,
    opacity: 0,
    duration: 0.4
})

tl.from(".center-part-1 button", {
    opacity: -1,
    duration: 0.4
})

tl.from(".center-part-2 img", {
    opacity: 0,
    duration: 0.3
}, "-=0.3")

tl.from(".section1bottom img", {
    opacity: 0,
    y: 30,
    stagger: 0.15,
})

var tl2 = gsap.timeline({
    scrollTrigger: {
        trigger: ".section2",
        scroller: "body",
        // markers: true,
        start: "top 50%",
        scrub: 2,
        end: "top -50%"
    }
});

tl2.from(".services", {
    y: 30,
    opacity: 0,
    duration: 0.5
})

tl2.from(".elem.line1.left", {
    x: -600,
    opacity: 0,
    duration: 1
}, "anim1")

tl2.from(".elem.line1.right", {
    x: 600,
    opacity: 0,
    duration: 1
}, "anim1")


tl2.from(".elem.line2.left", {
    x: -600,
    opacity: 0,
    duration: 1
}, "anim2")

tl2.from(".elem.line2.right", {
    x: 600,
    opacity: 0,
    duration: 1
}, "anim2")


// var cursor = document.querySelector("#cursor");
// var body = document.querySelector("body");

// // Function to update cursor position
// function updateCursorPosition(dets) {
//     // Calculate the new position including the scroll offset
//     const scrollX = window.scrollX || body.scrollLeft;
//     const scrollY = window.scrollY || body.scrollTop;

//     gsap.to(cursor, {
//         x: dets.clientX + scrollX,
//         y: dets.clientY + scrollY,
//         duration: 0.6,
//     });
// }

// // Add event listeners to track mouse movement across the entire document
// body.addEventListener("mousemove", (dets) => {
//     updateCursorPosition(dets);
// });
